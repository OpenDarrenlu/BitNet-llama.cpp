...
## arm
```bash
export MODEL_DIR="~/Downloads/~test_models" # your model dir
...
## x86
```bash
$env:MODEL_DIR="D:\Data\models"  # your model dir

pip install -r requirements.txt
pip install .\gguf-py
python convert-hf-to-gguf-bitnet.py $env:MODEL_DIR\bitnet_b1_58-large --outtype tl1
python convert-hf-to-gguf-bitnet.py $env:MODEL_DIR\bitnet_b1_58-large --outtype f32

python codegen_tl2.py --BMEMD 256 --BKEMD 96 --bmEMD 32 --byEMD 6 --BMGQA 128 --BKGQA 96 --bmGQA 32 --byGQA 4
cmake -B build -DGGML_BITNET_X86_TL2=ON -T ClangCL
cmake --build build --target llama-cli llama-bench llama-quantize llama-eval-callback --config Release
.\build\bin\Release\llama-quantize $env:MODEL_DIR\bitnet_b1_58-large\ggml-model-f32.gguf $env:MODEL_DIR\bitnet_b1_58-large\ggml-model.i2_s.gguf I2_S 1
.\build\bin\Release\llama-cli.exe -m "$env:MODEL_DIR\bitnet_b1_58-large\ggml-model-tl2.gguf" -n 128 -p "Microsoft Corporation is" -ngl 0 -c 2048 -b 1 -s 0 -t 1
.\build\bin\Release\llama-cli.exe -m "$env:MODEL_DIR\bitnet_b1_58-large\ggml-model-tl2.gguf" -n 128 -p "Microsoft Corporation is" -ngl 0 -c 2048 -b 1 -s 0 -t 1
```